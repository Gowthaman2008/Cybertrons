...
export const CommitsGrid = ({ text }: { text: string }) => {
  ...
  const getRandomColor = () => {
    const commitColors = [
      "#39ff14", // Ultra Neon Green
      "#00ff66", // Bright Emerald Neon
      "#4ade80"  // Active green
    ];
  ...
  return (
    <section
      className="w-full max-w-xl bg-base-surface border border-base-border grid p-1.5 sm:p-3 gap-0.5 sm:gap-1 rounded-[10px] sm:rounded-[15px]"
      style={{
      ...
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        return (
          <div
            key={index}
            className={cn(
              `border border-base-border/30 h-full w-full aspect-square rounded-[4px] sm:rounded-[3px]`,
              isHighlighted ? "animate-highlight" : "",
              shouldFlash ? "animate-flash" : "",
              !isHighlighted && !shouldFlash ? "bg-base-surface" : ""
            )}
            style={
              {
